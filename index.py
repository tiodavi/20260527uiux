...
<body class="bg-gray-100 p-6 min-h-screen flex flex-col items-center justify-start gap-6">
  ...
  <form action="/" method="POST" class="w-full max-w-md bg-[#3A3A3A] text-white p-4 rounded-lg shadow-lg font-sans">
    ...
    <div class="bg-[#1D4ED8] px-4 py-2 rounded-t-md flex justify-between items-center mb-4">
      <span class="text-sm font-bold">列車時刻查詢 (V3 頂規版)</span>
...
def index():
    ...
    if request.method == 'POST':
        ...
        if token:
            # 🎯 【終極修正網址】TDX V3 正式環境專用的起訖站核心路由：TrainOD
            api_url = f"https://tdx.transportdata.tw/api/basic/v3/Rail/TRA/DailyTrainTimetable/TrainOD/{form_data['start_station']}/To/{form_data['end_station']}/{form_data['search_date']}"
            ...
            try:
                ...
                if api_res.status_code == 200:
                    ...
                    for train in raw_data:
                        ...
                        t_type = info.get('TrainTypeName', {}).get('Zh_tw', '自強/區間')
                        
                        if len(stop_times) >= 2:
                            filtered_trains.append({
                                'train_type': t_type,
                                'train_no': t_no,
                                'dept_time': stop_times[0].get('DepartureTime', '00:00'),
                                'arr_time': stop_times[1].get('ArrivalTime', '00:00')
                            })
